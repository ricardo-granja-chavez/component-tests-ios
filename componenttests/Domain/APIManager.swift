...
class APIManager {
    ...
    public func requestImage(urlString: String, completion: @escaping (Result<UIImage?, APIError>) -> Void) {
        ...
        urlSession.dataTask(with: request) { (data, response, error) in
            if let httpResponse = response as? HTTPURLResponse {
                ...
                debugPrint("URL: [\(statusCode)] \(urlString)")
                if let data = data {
                    let dataString = String(data: data, encoding: .utf8) ?? "No data"
                    debugPrint("DATA: \(dataString)")
                }
                ...
                switch statusCode {
                case 100...199:
                    completion(.failure(.apiDefinedError(error: "code 100...199")))
                case 200...299:
                    guard let data = data else { return }
                    completion(.success(UIImage(data: data)))
                case 300...399:
                    completion(.failure(.apiDefinedError(error: "code 300...399")))
                case 400...499:
                    if let data = data {
                        let dataString = String(data: data, encoding: .utf8) ?? "No data"
                        completion(.failure(.apiDefinedError(error: dataString)))
                ...
                case 500...599:
                    completion(.failure(.apiDefinedError(error: "code 500...599")))
                default:
                    completion(.failure(.apiDefinedError(error: "server")))
                }
